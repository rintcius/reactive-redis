...
import net.cakesolutions.reactiveredis.driver.api.commands.{Ping, RedisCommand}
...
abstract class SubscriberBlackboxSpec(defaultTimeout: FiniteDuration)
  extends SubscriberBlackboxVerification[RedisCommand](new TestEnvironment(defaultTimeout.toMillis))
  ...
  override def createSubscriber(): Subscriber[RedisCommand] = {
    reactiveRedis(driver).subscriber()
  }

  def createHelperSource(elements: Long) : Source[RedisCommand, _] = elements match {
    case 0 => Source.empty
  ...
  override def createHelperPublisher(elements: Long): Publisher[RedisCommand] = {
    createHelperSource(elements).runWith(Sink.publisher)
  }

  override def createElement(i: Int): RedisCommand = Ping
}